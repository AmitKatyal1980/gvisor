...
func TestPortForwardStreamMode(t *testing.T) {
	...
	go func() {
		for {
			...
			upOut, upErr = server.Exec(ctx, dockerutil.ExecOpts{}, []string{"nginx"}...)
			if strings.Contains(upOut, "still could not bind()") {
				serverUpChan <- struct{}{}
				return
			}
		}
	...
	case <-time.After(time.Second * 10):
		t.Fatalf("could not verify servier is up: err: %v out: %s", upErr, upOut)
	}
